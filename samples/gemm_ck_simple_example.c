...
void printUsage(const char* programName)
{
    ...
    printf("                         CK GEMM only supports GFX12 (e.g. gfx1200, gfx1201)\n");
...
int main(int argc, char* argv[])
{
    ...
    printf("=== GEMM CK MxN Simple Example ===\n");
    printf("Verbose level set to: %d\n\n", verboseLevel);

    /*
     * GEMM CK MxN constraints (mirrors dxcp::CKGemm):
     *   - GFX12x architecture only
     *   - dataType = FLOAT32 (precision must be FLOAT32 or default)
     *   - transA = false, transB = false (NN only)
     *   - M % 128 == 0, N % 128 == 0, K % 32 == 0
     *   - M >= 128, N >= 128, K >= 64
     *   - alpha = 1.0 (fused into the kernel)
     *   - hasC = false, batch = 1
     *   - activation must be IDENTITY (or default)
     *
     * The 1024x1024x1024 NN FP32 problem chosen below satisfies all CK
     * constraints. Because dataType = FLOAT32 fails the HIP backend
     * (which requires FLOAT16), the dispatch falls back to the CK backend.
     */
    ...
    MLSSbool    transA       = 0;
    MLSSbool    transB       = 0;
    MLSSenum    dataType     = MLSS_FLOAT32;
    MLSSenum    precision    = MLSS_PRECISION_FLOAT32;
