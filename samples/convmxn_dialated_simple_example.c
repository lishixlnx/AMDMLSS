...
int main(int argc, char* argv[])
{
    ...
    MLSSuint32 w = 38;                   // Input x-dimension size.
    MLSSuint32 h = 68;                   // Input y-dimension size.
    MLSSuint32 c = 256;                   // Number of channels.
    MLSSuint32 n = 1;                   // Number of batches.
    MLSSuint32 k = 256;                   // Number of features.
    ...
    MLSSuint32 dilationX = 2;            // dilation X  
    MLSSuint32 dilationY = 2;            // dilation Y 
    MLSSuint32 startPadX = 2;            // Zero padding added to the beginning of the input in the x-dimension.
    MLSSuint32 startPadY = 2;            // Zero padding added to the beginning of the input in the y-dimension.
    MLSSuint32 endPadX = 2;              // Zero padding added to the end of the input in the x-dimension.
    MLSSuint32 endPadY = 2;              // Zero padding added to the end of the input in the y-dimension.
    ...
    MLSSuint32 filterStrideX = 2;        // Step between dot products in the filter x-dimension. Adds zero padding in the filter.
    MLSSuint32 filterStrideY = 2;        // Step between dot products in the filter y-dimension. Adds zero padding in the filter.
    ...
    CHECK_STATUS(mlssSetParameterByEnum(&context, opName, MLSS_ATTR_CONV_DILATIONX, &dilationX));
    CHECK_STATUS(mlssSetParameterByEnum(&context, opName, MLSS_ATTR_CONV_DILATIONY, &dilationY));
