...
namespace mlss::shaders::mha::ck::wmma
{
        ...
        const std::uint32_t& dataType)
    {
        ...
        auto setBlobs = [batchSize, headCount, kvSequenceLength, qSequenceLength, headDim, &binaries, &calcGridAndBlocks]<std::uint32_t MPerBlock, std::uint32_t NPerBlock, std::uint32_t BlockSize>(const auto& shaderRel, const auto& shaderNonRel, const auto& shaderRelWithStrides, const auto& shaderNonRelWithStrides, const auto& constants, const auto& arg_constants, const auto& arg_constants_with_strides)
            -> void
        {
            Blob relBlob;
            Blob nonRelBlob;
